...
export default function AllCases() {
  ...
  const handlePayFeeFunc = async (notificationId: any) => {
    try {
      ...
      const res = await axios.post(`/api/api/sendPayment`, {
        email: "zeeshanmnawaz@gmail.com",
        amount: 100,
      });

      console.log("🚀 ~ handlePayFeeFunc ~ res:", res);
      if (res.data.url) {
      ...
      alert("Case active successfully!");
      setSelectedCase(null);
    } catch (err) {
      console.error("Failed to case status as active:", err);
      alert("Failed to case status as active. Please try again.");
    }
  };
  return (
            ...
            <div className="p-8 h-full flex flex-col">
              ...
              <div className="mt-4 flex justify-end space-x-4">
                {selectedCase.status === "Accepted" ? (
                  <button
                    onClick={() => {
                      handlePayFeeFunc(selectedCase._id);
                    }}
